        ...
        <DatepickerCtx.Provider value={ctxValue}>
            <Manager>
                <Reference>
                    {({ ref }) => (
                        <label
                            className="flex flex-col text-gray-700 text-sm font-bold"
                            htmlFor={id}
                        >
                            {label}
...
export const DatePicker: React.FC<DatePickerProps> = ({ label, id, date, onChange }) => (
    <RawDatePicker label={label} id={id} date={date} onChange={onChange} />
);
